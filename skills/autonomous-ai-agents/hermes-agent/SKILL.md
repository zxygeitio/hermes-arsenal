...
For custom OpenAI-compatible providers, prefer model-specific context lengths under `custom_providers[].models.<model>.context_length` rather than a broad top-level `model.context_length` override. Top-level `model.context_length` wins and masks provider-specific values. See `references/custom-provider-context-length.md`.

### Dual-Model Delegation Strategy

When you have two models with complementary strengths (e.g. large-context + precision), configure `model.default` for the control plane and `delegation.model` for the scalpel. Full walkthrough and config template: `references/dual-model-delegation-strategy.md`.

### Config Writing — Key Path Resolution

`hermes config set KEY VAL` does **flattened key lookup** in the YAML, not nested key creation.

**What works:** Top-level keys (`hermes config set model.default claude-sonnet-4`) and single-level nested keys that exist as a literal line in config.yaml (`hermes config set display.skin default`).

**What breaks:** Setting a nested key whose parent section exists but whose full `parent.key: value` line does NOT already exist in the file as its own line.

**Safe pattern for new nested keys:** Edit the file directly with patch, or verify with `grep -n "key:" ~/.hermes/config.yaml` first.

If a config set command produces unexpected results, always verify with:
```bash
grep -n "your_key\|your_section" ~/.hermes/config.yaml
```
...
inside an `execute_code` block to confirm `SYSTEMROOT` is set. If a local `references/execute-code-sandbox-env-windows.md` exists in this skill, read it for the full diagnostic recipe; otherwise use the inline checks above and inspect `tools/code_execution_tool.py` in the Hermes repo.

### Testing / Contributing

**Repo script note:** `scripts/run_tests.sh` in the Hermes source repo doesn't work as-is on Windows — it looks for
...
Full design notes, exact emitted strings, and testing pitfalls may live in a repo-side design note named `references/prompt-builder-environment-hints.md`; if that file is not present, rely on `agent/prompt_builder.py::build_environment_hints()` and `tests/agent/test_prompt_builder.py::TestEnvironmentHints` as the source of truth.
